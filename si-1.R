...
with(profile, points(logLik[which.max(logLik)] ~ x[which.max(logLik)], pch=20, col="red", cex=3))
cis <- order(with(profile, abs(logLik - (max(logLik)-2))))[1:2] # Checked from model output; uses 2x log-unit rule (uncomment abline below to confirm locations)
with(profile, arrows(x[cis[1]], logLik[cis[1]], x[cis[2]], logLik[cis[2]], col="orange", lwd=3, code=3, angle=90, length=.2))
#with(profile, text(x[which.max(logLik)],logLik[which.max(logLik)]+.5, "Maximum Likelihood\nEstimate", col="red", xpd=NA))
#with(profile, text(median(x[cis]), median(logLik[cis])-.5, "95% CIs", col="orange"))
with(profile, text(x[which.max(logLik)], -131.5, "Estimate", col="red", xpd=NA))
with(profile, text(.5, -132, "95% CIs", col="orange"))
with(profile, arrows(x[cis[1]], -132, x[cis[2]], -132, col="orange", lwd=3, code=3, angle=90, length=.2))
with(profile, points(x[which.max(logLik)], -132, pch=20, col="red", cex=3, xpd=NA))
